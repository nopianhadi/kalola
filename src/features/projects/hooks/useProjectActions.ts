...
export const useProjectActions = ({
    ...
    const handleFormSubmit = useCallback(async (e: React.FormEvent) => {
        ...
        // Helper: sync team_project_payments from team assignments
        const syncTeamPayments = async (projectId: number, team: any[], projectDate: string) => {
            if (!team || team.length === 0) return;
            const payments: TeamProjectPayment[] = team.map((t: any) => ({
                id: 0, // will be generated by backend upsert
                projectId,
                teamMemberId: Number(t.memberId),
            ...
            await upsertTeamPaymentsForProject(projectId, payments);
        };

        try {
            if (formMode === 'add') {
                const created = await createProjectWithRelations(projectData);
                // Sync team assignments + payments for new project
                if (created?.id && (projectData.team || []).length > 0) {
                    ...
                    await upsertAssignmentsForProject(created.id, newTeam);
                    await syncTeamPayments(created.id, projectData.team, projectData.date);
                }
                ...
                // Update assignments and sync team_project_payments
                const newTeam = (projectData.team || []).map((t: any) => ({
                    memberId: t.memberId,
                    name: t.name,
                    role: t.role,
                ...
                await syncTeamPayments(projectData.id, projectData.team || [], projectData.date);
                queryClient.invalidateQueries();
