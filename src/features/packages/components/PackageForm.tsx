...
export const PackageForm: React.FC<PackageFormProps> = ({
    ...
    profile, unionRegions, inline = false, onCancel
}) => {
    ...
    const formContent = (
        <form onSubmit={onSubmit} className={`space-y-6 ${!inline ? 'max-h-[75vh] overflow-y-auto px-1 pb-4 custom-scrollbar' : ''}`}>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="input-group">
                            ...
                            <input type="text" name="region" list="region-suggestions-pkg-inline" value={formData.region} onChange={onInputChange} className="w-full px-4 py-3 rounded-xl border border-brand-border bg-white" placeholder="Cth: Jakarta" />
                            <datalist id="region-suggestions-pkg-inline">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center p-3 bg-brand-bg/30">
                                ...
                                <div className="md:col-span-4 flex items-center justify-end gap-2 text-xs">
                                    <label className="flex items-center gap-1.5 cursor-pointer font-bold text-brand-text-secondary">
                                        <input type="radio" name={`durationDefault-${index}`} checked={!!opt.default} onChange={() => onDurationOptionChange(index, 'default', true)} className="text-blue-600" /> Default
                                    </label>
            ...
            <div className={`flex justify-end gap-3 pt-6 mt-4 border-t border-brand-border ${!inline ? 'sticky -bottom-4 bg-white/90 backdrop-blur-xl p-4 -mx-1' : ''}`}>
                <button type="button" onClick={onCancel} className="px-8 py-3 rounded-xl font-bold text-brand-text-secondary hover:bg-brand-bg transition-colors">Batal</button>
