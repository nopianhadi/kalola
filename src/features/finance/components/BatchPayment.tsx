...
import { TeamProjectPayment, Card, FinancialPocket, PaymentStatus } from '@/types';
import { CheckSquareIcon, DollarSignIcon, AlertCircleIcon } from '@/constants';

interface BatchPaymentProps {
    payments: TeamProjectPayment[];
    cards: Card[];
    pockets: FinancialPocket[];
    onBatchPay: (paymentIds: number[], sourceCardId: number, sourcePocketId?: number) => Promise<void>;
...
export const BatchPayment: React.FC<BatchPaymentProps> = ({
    ...
    const [sourcePocketId, setSourcePocketId] = useState<number | ''>('');
    const [isProcessing, setIsProcessing] = useState(false);

    // Filter unpaid payments
    const unpaidPayments = useMemo(() =>
        payments.filter(p => p.status === PaymentStatus.BELUM_BAYAR),
        [payments]
    );
    ...
    const availableBalance = useMemo(() => {
        if (paymentType === 'card' && sourceCardId) {
            const card = cards.find(c => String(c.id) === String(sourceCardId));
            return card?.balance || 0;
        }
        if (paymentType === 'pocket' && sourcePocketId) {
            const pocket = pockets.find(p => String(p.id) === String(sourcePocketId));
    ...
    const handleBatchPay = async () => {
        ...
        try {
            await onBatchPay(
                ...
                Number(sourceCardId),
                paymentType === 'pocket' && sourcePocketId ? Number(sourcePocketId) : undefined
            );
            setSelectedPaymentIds(new Set());
            setSourceCardId('');
            setSourcePocketId('');
                    ...
                    <div className="space-y-3">
                        ...
                        {paymentType === 'card' ? (
                            ...
                            <select
                                ...
                                onChange={(e) => setSourcePocketId(e.target.value ? Number(e.target.value) : '')}
                                className="w-full px-4 py-3 rounded-xl border border-brand-border bg-white/5 text-brand-text-primary focus:outline-none focus:ring-2 focus:ring-brand-accent"
                            >
