...
const sanitizeImageUrl = (url: string): string => {
    if (!url) {
        console.warn('[Security] Empty image URL');
        return '/placeholder-image.jpg';
    }

    // Allow base64 data URLs (for images stored in database)
        ...
        try {
            ...
            console.log('Gallery Data Loaded:', {
                ...
                firstImage: galleryData?.images?.[0] ? {
                    id: galleryData.images[0].id,
                    hasUrl: !!galleryData.images[0].url,
                    hasThumbnail: !!galleryData.images[0].thumbnailUrl,
                    urlPreview: galleryData.images[0].url?.substring(0, 50) + '...'
                } : null
            });
                        ...
                        <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-0">
                            {gallery.images.map((image, index) => {
                                ...
                                console.log(`Image ${index + 1}:`, {
                                    ...
                                    urlType: imageUrl?.startsWith('data:') ? 'base64' : imageUrl?.startsWith('http') ? 'http' : 'unknown',
